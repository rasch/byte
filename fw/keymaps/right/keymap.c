...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  [_DEF] = LAYOUT(
    A_BRC, KC_R, KC_T, S_NUM,
    E_SYM, KC_Y, KC_I, O_MSC
  ),
  [_NUM] = LAYOUT(
  ...
  [_BRC] = LAYOUT(
    XXXXXXX, LSFT(KC_9), KC_LBRC, LSFT(KC_LBRC),
    XXXXXXX, LSFT(KC_0), KC_RBRC, LSFT(KC_RBRC)
  ),
  [_SYM] = LAYOUT(
    LSFT(KC_3), KC_GRV, KC_SCLN, KC_BSLS,
    XXXXXXX, LSFT(KC_2), KC_MINS, KC_EQL
  ),
  [_MSC] = LAYOUT(
    KC_MPLY, KC_MUTE, KC_VOLU, XXXXXXX,
    KC_MPRV, KC_MNXT, KC_VOLD, XXXXXXX
  ),
  [_NAV] = LAYOUT(
